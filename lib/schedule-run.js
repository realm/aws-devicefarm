...
exports.scheduleRun = async function (params) {
    ...
    if (params.testSpecFile) {
        if (params.test_spec && params.test_spec.length) {
            var spec = fs.createWriteStream(params.testSpecFile);
            spec.on('error', function(err){
                throw("Error writing test spec: " + err)
            });
            params.test_spec.forEach(function(line) { spec.write(line + '\n'); });
            spec.end();
        }
